...
namespace Profunia.Inventory.Desktop.Transactions
{
    public partial class frmSalesOrder : Form
    {
        ...
        public void CheckInvalidEntries(DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (dgvSalesOrder.CurrentRow != null)
                {
                    if (!isValueChanged)
                    {
                        if (spSettings.SettingsStatusCheck("AllowZeroValueEntry") == "No" && Convert.ToDecimal(dgvSalesOrder.CurrentRow.Cells["dgvtxtRate"].Value) == 0)
                        {
                            isValueChange = true;
        ...
        private void txtDate_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("SO67:" + ex.Message, "OpenMiracle", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        /// <summary>
        /// Due date validation
        ...
        private void txtDueDate_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("SO68:" + ex.Message, "OpenMiracle", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        /// <summary>
        ...
        public void productDetailsFill(string strProduct, int inRowIndex, string strFillMode)
        {
            try
            {
                ...
                if (dtbl.Rows.Count != 0)
                {
                    ...
                    UnitConvertionSP SpUnitConvertion = new UnitConvertionSP();
                    DataTable dtblUnitByProduct = new DataTable();
                    dtblUnitByProduct = SpUnitConvertion.UnitConversionIdAndConRateViewallByProductId(dgvSalesOrder.Rows[inRowIndex].Cells["dgvtxtProductCode"].Value.ToString());
                    foreach (DataRow drUnitByProduct in dtblUnitByProduct.Rows)
                    {
